...
import de from "../../../../mail-app/translations/de"
...
export class OfflineStorage implements CacheStorage, ExposedCacheStorage {
	...
	async getRawListElementsOfType(typeRef: TypeRef<ListElementEntity>): Promise<Array<ListElementEntity>> {
		...
		const typeModel = await resolveTypeReference(typeRef)
		return items.map((item) => this.decodeCborEntity(item.entity.value as Uint8Array, typeModel) as Record<string, unknown> & ListElementEntity)
	}
	...
	async getRawElementsOfType(typeRef: TypeRef<ElementEntity>): Promise<Array<ElementEntity>> {
		...
		const items = (await this.sqlCipherFacade.all(query, params)) ?? []
		const typeModel = await resolveTypeReference(typeRef)
		return items.map((item) => this.decodeCborEntity(item.entity.value as Uint8Array, typeModel) as Record<string, unknown> & ElementEntity)
	}
	...
	private async serialize(originalEntity: SomeEntity): Promise<Uint8Array> {
		const typeModel = await resolveTypeReference(originalEntity._type)
		const idMappedInstance: Record<number, any> = await new InstanceMapper().mapToLiteral(originalEntity, typeModel)
		try {
	...
	private async deserialize<T extends SomeEntity>(typeRef: TypeRef<T>, loaded: Uint8Array): Promise<T | null> {
		...
		let deserialized
		try {
			deserialized = this.decodeCborEntity(loaded, typeModel)
		} catch (e) {
			console.log(e)
	...
	private decodeCborEntity(loaded: Uint8Array, typeModel: TypeModel): Record<string, unknown> {
