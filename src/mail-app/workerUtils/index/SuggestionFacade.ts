	...
	load(): Promise<void> {
		return this._db.initialized.then(() => {
			return this._db.dbFacade.createTransaction(true, [SearchTermSuggestionsOS]).then((t) => {
				return t.get(SearchTermSuggestionsOS, this.type.typeId.toLowerCase()).then((encSuggestions) => {
					if (encSuggestions) {
	...
	store(): Promise<void> {
		return this._db.initialized.then(() => {
			return this._db.dbFacade.createTransaction(false, [SearchTermSuggestionsOS]).then((t) => {
				...
				t.put(SearchTermSuggestionsOS, this.type.typeId.toLowerCase(), encSuggestions)
				return t.wait()
			})
		})
	}
